...
xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
        ...
        $.each(datos, function (i, item) {
            let name = item.programStages[i].displayName;

            let prgrmSS = item.programStages[i].sections;
            let cnt_prgrmSS = prgrmSS.length;
            let id = item.programStages[i].id;

            console.log("Secciones totales: " + cnt_prgrmSS);

            let c_error = item.msg_errors.length;
            let c_warning = item.msg_warnings.length;
            for (let s = 0; s < cnt_prgrmSS; s++) {
                console.log("S: " + s);
                console.log("Leo secciones: " + item.programStages[i].sections[s].displayName);
                ...
                if (c_error == 0 & c_warning == 0) {
                    ...
                    <div class="cmpt_program_bsct">
                        ...
                        <div class="program_bsct-title">
                            ${name} <span>(${cnt_prgrmSS}) Program stage sections</span>
                        </div>
                ...
                if (c_error != 0 & c_warning != 0) {
                    ...
                    <div class="cmpt_program_bsct">
                        ...
                        <div class="program_bsct-title">
                            ${name} <span>(${cnt_prgrmSS}) Program stages sections</span>
                        </div>
